...
import { getProductsByStoreId, getServicesByStoreId, getCustomersByStoreId, addTransaction, addCustomer as addCustomerFirestore } from "@/lib/firestoreUtils";
import type { Product, Service, CartItem, Customer, TransactionItem } from "@/types"; // Removed Store from here as it comes from UserContext
...
// Simulate a terminal ID - in a real app this might come from device info or login context
const SIMULATED_TERMINAL_ID = `TERM-${Math.random().toString(36).substring(2, 7).toUpperCase()}`;


export default function SalesPage() {
  const { user, userDoc, storeDetails } = useUser();
  ...
  React.useEffect(() => {
    if (userDoc?.storeId) {
      const fetchInitialData = async () => {
        ...
        try {
          // For large catalogs (e.g., 50k items), fetching ALL products/services here isn't ideal.
          // Better strategies: fetch by default category, fetch 'favorites', or encourage search first.
          // Firestore caches query results, so subsequent identical queries are fast offline.
          // The current implementation fetches all visible products/services.
          ...
          const activeProducts: CatalogDisplayItem[] = fetchedProducts
            .filter(p => p.isVisibleOnPOS && p.stockQuantity > 0) // Also ensure stock > 0 for products initially
  ...
  const calculateDiscount = (currentSubtotal: number, promoKey: string | null): number => {
    ...
    let discount = 0;
    if (promoDetails.type === 'fixed') discount = promoDetails.value;
    else if (promoDetails.type === 'percentage') discount = currentSubtotal * promoDetails.value;
  ...
  const handleApplyOrRemovePromoCode = () => {
    if (appliedPromoCode && promoCodeInput === "") {
        setAppliedPromoCode(null); setAppliedDiscountAmount(0);
        toast({ title: "Promo Removed", description: "Discount has been removed from the order."});
        return;
    }
    const code = promoCodeInput.trim().toUpperCase();
    if (!code) { toast({ title: "No Code", description: "Please enter a promo code.", variant: "default"}); return; }
  ...
  const handlePaymentMethodSelect = (method: 'cash' | 'card') => {
    setSelectedPaymentMethod(method); setCurrentStep('payment'); setAmountTendered(""); setReceiptSent(false);
    const customer = customers.find(c => c.id === selectedCustomerId);
    if (customer) setReceiptRecipient(customer.email || customer.phone || ""); else setReceiptRecipient("");
  };

  const handleProcessPayment = async () => {
    if (cartItems.length === 0) { toast({ title: "Empty Cart", description: "Add items to cart first.", variant: "default" }); return; }
    if (!selectedPaymentMethod) { toast({ title: "No Payment Method", description: "Please select a payment method.", variant: "default" }); return; }
    if (selectedPaymentMethod === 'cash' && parseFloat(amountTendered || "0") < total) {
        toast({ title: "Insufficient Cash", description: "Amount tendered is less than total.", variant: "destructive" }); return;
    }
    setIsProcessingOrSending(true);
    await new Promise(resolve => setTimeout(resolve, selectedPaymentMethod === 'cash' ? 1000 : 2000)); 
    setIsProcessingOrSending(false); setCurrentStep('receipt');
  ...
  const handleSimulatedSendReceipt = async (channel: 'SMS' | 'Email') => {
    ...
    setIsProcessingOrSending(false); setReceiptSent(true);
    toast({ title: "Receipt Sent (Simulated)", description: `Digital receipt sent via ${channel} to ${receiptRecipient}.` });
  };

  const handleNoReceipt = () => {
    setReceiptSent(true); 
    toast({ title: "No Receipt", description: "Customer opted out of a digital receipt.", variant: "default" });
  }

  const handleFinalizeSale = async () => {
    if (!userDoc || !user || !storeDetails) {
      toast({ title: "Error", description: "User or store information not available.", variant: "destructive" }); return;
    }
    setIsProcessingOrSending(true);
    try {
      const customerForTx = customers.find(c => c.id === selectedCustomerId);
      const transactionPromise = addTransaction(
        userDoc.storeId, user.uid, userDoc.displayName, cartItems, 
        subtotal, tax, total, selectedPaymentMethod || "other", 
        SIMULATED_TERMINAL_ID, // Pass simulated terminal ID
        selectedCustomerId, customerForTx?.name,
        appliedDiscountAmount, appliedPromoCode
      );
      ...
      const [refreshedProducts, refreshedServices] = await Promise.all([
          getProductsByStoreId(userDoc.storeId), getServicesByStoreId(userDoc.storeId),
      ]);
      setProducts(refreshedProducts); setServices(refreshedServices);
      const activeProducts: CatalogDisplayItem[] = refreshedProducts.filter(p => p.isVisibleOnPOS).map(p => ({ ...p, itemType: 'product' as const }));
      const activeServices: CatalogDisplayItem[] = refreshedServices.filter(s => s.isVisibleOnPOS).map(s => ({ ...s, itemType: 'service' as const }));
      setCatalogItems([...activeProducts, ...activeServices].sort((a,b) => a.name.localeCompare(b.name)));
  ...
  const handleSelectCustomer = (customer: Customer) => {
    setSelectedCustomerId(customer.id); setSelectedCustomerName(customer.name);
    setReceiptRecipient(customer.email || customer.phone || ""); 
    setIsCustomerModalOpen(false); setCustomerSearchTerm(""); setCustomerModalView('search');
    newCustomerForm.reset();
    toast({ title: "Customer Assigned", description: `${customer.name} assigned to this transaction.`});
  };

  const handleClearCustomer = () => {
    setSelectedCustomerId(undefined); setSelectedCustomerName(undefined); setReceiptRecipient(""); 
  ...
  const filteredCustomersForModal = customers.filter(customer => {
    ...
    return ( customer.name.toLowerCase().includes(search) || (customer.email && customer.email.toLowerCase().includes(search)) || (customer.phone && customer.phone.includes(search)) );
  });

  const isItemInCart = (itemId: string, itemType: 'product' | 'service') => cartItems.some(cartItem => cartItem.productId === itemId && cartItem.itemType === itemType);
  ...
  const handleOpenCustomerModal = () => { setCustomerModalView('search'); newCustomerForm.reset(); setIsCustomerModalOpen(true); }

  const onSubmitNewCustomer = async (data: NewCustomerFormValues) => {
    if (!userDoc?.storeId) { toast({ title: "Error", description: "Store ID not found.", variant: "destructive" }); return; }
    setIsAddingCustomer(true);
    try {
        const newCustomerId = await addCustomerFirestore(userDoc.storeId, { name: data.name, email: data.email, phone: data.phone, notes: data.notes });
        const newCustomer: Customer = {
            id: newCustomerId, storeId: userDoc.storeId, name: data.name, email: data.email || "", phone: data.phone || "", notes: data.notes || "",
            totalSpent: 0, loyaltyPoints: 0, createdAt: new Date() as any, lastUpdatedAt: new Date() as any,
        };
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-3 p-4">
              {filteredCatalogItems.map(item => {
                ...
                <Card 
                  ...
                  role="button" tabIndex={isOutOfStock || isProcessingOrSending ? -1 : 0}
                  onKeyDown={(e) => !isOutOfStock && !isProcessingOrSending && e.key === 'Enter' && addToCart(item)}
                  aria-label={`Add ${item.name} to cart`} aria-disabled={isOutOfStock || isProcessingOrSending}
                >
                  ...
                  {item.imageUrl ? (
                    <Image src={item.imageUrl} alt={item.name} width={60} height={60} className="rounded-md mb-2 object-cover" data-ai-hint={item.itemType === 'service' ? "service icon" : "product item"}/>
      ...
      <div className="flex-grow-[2] p-4 flex flex-col border-r border-border bg-muted/20 dark:bg-muted/10 overflow-hidden basis-2/5">
            ...
            <div className="flex items-center gap-1">
                 {lastCartAction && (
                    <TooltipProvider delayDuration={100}> <Tooltip> <TooltipTrigger asChild>
                        <Button variant="ghost" size="icon" onClick={handleUndoLastCartAction} className="text-muted-foreground hover:text-foreground" disabled={isProcessingOrSending}> <Undo2 className="h-5 w-5"/> </Button>
                    </TooltipTrigger> <TooltipContent side="bottom" className="bg-popover text-popover-foreground"> <p>Undo Last Action</p> </TooltipContent> </Tooltip> </TooltipProvider>
                )}
                {cartItems.length > 0 && (
                    ...
                    </AlertDialogTrigger> <AlertDialogContent> <AlertDialogHeader> <AlertDialogTitle>Clear Cart?</AlertDialogTitle> <AlertDialogDescription> Are you sure you want to remove all items and any applied promo code from the current order? The assigned customer (if any) will remain. </AlertDialogDescription> </AlertDialogHeader> <AlertDialogFooter> <AlertDialogCancel>Cancel</AlertDialogCancel> <AlertDialogAction onClick={() => resetTerminalState(false, true)} className="bg-destructive hover:bg-destructive/90 text-destructive-foreground">Clear Cart</AlertDialogAction> </AlertDialogFooter> </AlertDialogContent> </AlertDialog>
                )}
            </div>
          </CardTitle>
        </CardHeader>
          ...
          <div className="px-4 divide-y divide-border">
            {cartItems.map(item => (
              <div key={`${item.productId}-${item.itemType}`} className="py-3 flex items-center gap-3">
                {item.imageUrl ? ( <Image src={item.imageUrl} alt={item.name} width={40} height={40} className="rounded-md object-cover" data-ai-hint="cart item" />
                ) : item.itemType === 'service' ? ( <ConciergeBell className="h-8 w-8 text-primary rounded-md" />
                ...
                <div className="flex-grow">
                  ...
                  <div className="flex items-center mt-1 gap-1">
                     ...
                     <Input type="number" value={item.quantity} onChange={(e) => setExactQuantity(item.productId, item.itemType!, parseInt(e.target.value) || 0)} onFocus={(e) => e.target.select()} min="0" className="h-7 w-12 text-sm text-center p-1" aria-label={`Quantity for ${item.name}`} disabled={isProcessingOrSending}/>
                ...
                <p className="font-semibold text-foreground w-20 text-right">${item.totalPrice.toFixed(2)}</p>
                 <AlertDialog> <AlertDialogTrigger asChild>
                    <Button variant="ghost" size="icon" className="ml-1 text-muted-foreground hover:text-destructive" aria-label={`Remove ${item.name} from cart`} disabled={isProcessingOrSending}> <Trash2 className="h-4 w-4"/> </Button>
                  </AlertDialogTrigger> <AlertDialogContent> <AlertDialogHeader> <AlertDialogTitle>Remove Item?</AlertDialogTitle> <AlertDialogDescription> Are you sure you want to remove "{item.name}" from the cart? </AlertDialogDescription> </AlertDialogHeader> <AlertDialogFooter> <AlertDialogCancel>Cancel</AlertDialogCancel> <AlertDialogAction onClick={() => removeFromCart(item.productId, item.itemType!)} className="bg-destructive hover:bg-destructive/90 text-destructive-foreground"> Remove </AlertDialogAction> </AlertDialogFooter> </AlertDialogContent> </AlertDialog>
              </div>
            ))}
        ...
        <div className="pt-4 mt-auto">
            <div className="flex items-end gap-2 mb-3">
                ...
                <Button variant="outline" className="h-10 shrink-0 text-foreground hover:bg-accent hover:text-accent-foreground" onClick={() => toast({ title: "Coming Soon!", description: "Manual discount functionality will be added here."})} disabled={isProcessingOrSending || currentStep !== 'order' || cartItems.length === 0}>
                  <Tag className="mr-2 h-4 w-4" /> Add Discount
                </Button>
                <Button onClick={handleApplyOrRemovePromoCode} className="h-10 shrink-0" disabled={promoButtonDisabled || isProcessingOrSending || currentStep !== 'order'}> {promoButtonText} </Button>
            </div>
            ...
            <div className="mb-2">
                ...
                {selectedCustomerId && selectedCustomerName ? (
                    <div className="flex items-center justify-between p-2 border rounded-md bg-background h-10">
                        <Button variant="link" className="p-0 h-auto text-sm font-medium text-foreground flex items-center hover:no-underline" onClick={handleOpenCustomerModal} disabled={isProcessingOrSending || currentStep !== 'order'}> <UserCheck className="mr-2 h-4 w-4 text-primary"/>{selectedCustomerName} </Button>
                        <Button variant="ghost" size="icon" onClick={handleClearCustomer} className="h-7 w-7 text-muted-foreground hover:text-destructive" disabled={isProcessingOrSending || currentStep !== 'order'}> <UserX className="h-4 w-4" /> <span className="sr-only">Clear customer</span> </Button>
                    </div>
                ) : ( <Button variant="outline" className="w-full h-10 justify-start" onClick={handleOpenCustomerModal} disabled={isProcessingOrSending || currentStep !== 'order'}> <UserPlus className="mr-2 h-4 w-4 text-muted-foreground" /> Assign Customer </Button> )}
            </div>
            
            <Dialog open={isCustomerModalOpen} onOpenChange={setIsCustomerModalOpen}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader> <DialogTitle>{customerModalView === 'search' ? 'Assign Customer' : 'Add New Customer'}</DialogTitle> <DialogDescription> {customerModalView === 'search' ? 'Search for an existing customer or add a new one.' : 'Enter details for the new customer.'} </DialogDescription> </DialogHeader>
                    {customerModalView === 'search' && ( <>
                            <div className="relative mt-2 mb-4"> <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /> <Input placeholder="Search by name, email, phone..." className="pl-10" value={customerSearchTerm} onChange={(e) => setCustomerSearchTerm(e.target.value)} /> </div>
                            <ScrollArea className="h-[200px] border rounded-md">
                                {filteredCustomersForModal.length > 0 ? ( filteredCustomersForModal.map(customer => ( <div key={customer.id} className="p-2 hover:bg-accent cursor-pointer border-b last:border-b-0" onClick={() => handleSelectCustomer(customer)}> <p className="font-medium text-sm text-foreground">{customer.name}</p> <p className="text-xs text-muted-foreground">{customer.email || customer.phone || "No contact info"}</p> </div> ))
                                ) : ( <p className="p-4 text-center text-sm text-muted-foreground"> {customerSearchTerm ? "No customers match your search." : "No customers found."} </p> )}
                            </ScrollArea>
                            <DialogFooter className="sm:justify-between pt-4"> <Button type="button" variant="outline" onClick={() => setCustomerModalView('add')}> <UserPlus className="mr-2 h-4 w-4" /> Add New Customer </Button> <DialogClose asChild> <Button type="button" variant="secondary">Cancel</Button> </DialogClose> </DialogFooter>
                        </>
                    )}
                    {customerModalView === 'add' && (
                        <Form {...newCustomerForm}> <form onSubmit={newCustomerForm.handleSubmit(onSubmitNewCustomer)} className="space-y-4 pt-2">
                                <FormField control={newCustomerForm.control} name="name" render={({ field }) => ( <FormItem> <FormLabel>Name</FormLabel> <FormControl><Input placeholder="Customer's full name" {...field} /></FormControl> <FormMessage /> </FormItem> )}/>
                                <FormField control={newCustomerForm.control} name="email" render={({ field }) => ( <FormItem> <FormLabel>Email (Optional)</FormLabel> <FormControl><Input type="email" placeholder="customer@example.com" {...field} /></FormControl> <FormMessage /> </FormItem> )}/>
                                <FormField control={newCustomerForm.control} name="phone" render={({ field }) => ( <FormItem> <FormLabel>Phone (Optional)</FormLabel> <FormControl><Input type="tel" placeholder="+15551234567" {...field} /></FormControl> <FormMessage /> </FormItem> )}/>
                                <FormField control={newCustomerForm.control} name="notes" render={({ field }) => ( <FormItem> <FormLabel>Notes (Optional)</FormLabel> <FormControl><Textarea placeholder="Any relevant notes about the customer..." {...field} rows={2}/></FormControl> <FormMessage /> </FormItem> )}/>
                                <DialogFooter className="sm:justify-between pt-4"> <Button type="button" variant="outline" onClick={() => { setCustomerModalView('search'); newCustomerForm.reset();}}> Back to Search </Button> <Button type="submit" disabled={isAddingCustomer}> {isAddingCustomer && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Save & Assign Customer </Button> </DialogFooter>
                            </form> </Form>
                    )}
                </DialogContent>
            </Dialog>
            ...
            <div className="space-y-1 text-sm">
                ...
                {appliedDiscountAmount > 0 && appliedPromoCode && ( <div className="flex justify-between text-destructive"> <span className="text-destructive">Discount ({HARDCODED_PROMO_CODES[appliedPromoCode.toUpperCase()]?.description || appliedPromoCode})</span> <span className="font-medium text-destructive">-${appliedDiscountAmount.toFixed(2)}</span> </div> )}
                <div className="flex justify-between"> <span className="text-muted-foreground">Tax ({ (taxRate * 100).toFixed(0) }%)</span> <span className="font-medium text-foreground">${tax.toFixed(2)}</span> </div>
                <Separator />
                <div className="flex justify-between text-2xl font-bold pt-1"> <span className="text-foreground">Total</span> <span className="text-primary">${total.toFixed(2)}</span> </div>
            </div>
        </div>
      </div>
      ...
      <div className="flex-grow-[1] p-6 flex flex-col bg-card shadow-lg overflow-y-auto basis-1/5 min-w-[300px] md:min-w-[360px]">
        {currentStep === 'order' && ( <>
                ...
                <div className="grid grid-cols-1 gap-4 flex-grow content-start">
                    <Button onClick={() => handlePaymentMethodSelect('cash')} className="h-20 text-xl bg-green-600 hover:bg-green-700 text-white dark:bg-green-500 dark:hover:bg-green-600" disabled={cartItems.length === 0 || isProcessingOrSending}> <Wallet className="mr-3 h-8 w-8"/>Cash </Button>
                    <Button onClick={() => handlePaymentMethodSelect('card')} className="h-20 text-xl bg-blue-600 hover:bg-blue-700 text-white dark:bg-blue-500 dark:hover:bg-blue-600" disabled={cartItems.length === 0 || isProcessingOrSending}> <CreditCard className="mr-3 h-8 w-8"/>Card </Button>
        ...
        {currentStep === 'payment' && selectedPaymentMethod === 'cash' && ( <>
                ...
                <Input id="amount-tendered" type="number" placeholder="0.00" value={amountTendered} onChange={(e) => setAmountTendered(e.target.value)} className="h-16 text-3xl text-right mb-1" disabled={isProcessingOrSending}/>
                 {parseFloat(amountTendered || "0") < total && amountTendered !== "" && ( <p className="text-xs text-destructive text-right mb-2">Amount tendered is less than total.</p> )}
                <div className="grid grid-cols-2 gap-2 mb-3">
                    {quickTenderAmounts.map(amount => ( <Button key={amount} variant="outline" className="h-12 text-base" onClick={() => setAmountTendered(amount.toString())} disabled={isProcessingOrSending}>${amount}</Button> ))}
                ...
                <div className="text-right text-2xl font-bold my-4 p-3 bg-muted rounded-md"> <span className="text-muted-foreground">Change Due: </span> <span className="text-green-600 dark:text-green-500">${changeDue.toFixed(2)}</span> </div>
                <Button onClick={handleProcessPayment} className="w-full h-16 text-xl mt-auto bg-green-600 hover:bg-green-700 text-white dark:bg-green-500 dark:hover:bg-green-600" disabled={isProcessingOrSending || parseFloat(amountTendered || "0") < total || amountTendered === ""}>
                    {isProcessingOrSending && currentStep === 'payment' && selectedPaymentMethod === 'cash' ? <Loader2 className="mr-2 h-6 w-6 animate-spin" /> : "Process Cash"} </Button>
            </>
        )}
        {currentStep === 'payment' && selectedPaymentMethod === 'card' && ( <>
                ...
                <div className="flex flex-col items-center justify-center flex-grow text-center"> <CreditCard className="h-24 w-24 text-primary mb-4"/> <p className="text-muted-foreground mb-2">Please use card terminal to complete payment.</p> <p className="text-2xl font-bold text-foreground">Total: ${total.toFixed(2)}</p> </div>
                <Button onClick={handleProcessPayment} className="w-full h-16 text-xl mt-auto bg-blue-600 hover:bg-blue-700 text-white dark:bg-blue-500 dark:hover:bg-blue-600" disabled={isProcessingOrSending}>
                    {isProcessingOrSending && currentStep === 'payment' && selectedPaymentMethod === 'card' ? <Loader2 className="mr-2 h-6 w-6 animate-spin" /> : "Process Card"} </Button>
            </>
        )}
        {currentStep === 'receipt' && ( <>
                <h2 className="text-xl font-semibold mb-6 text-center text-foreground">Digital Receipt</h2>
                <div className="space-y-4 mb-6">
                    <div> <Label htmlFor="receipt-recipient" className="text-muted-foreground">Phone or Email</Label> <Input id="receipt-recipient" placeholder="Enter customer's phone or email" value={receiptRecipient} onChange={(e) => setReceiptRecipient(e.target.value)} className="h-12 text-lg" disabled={isProcessingOrSending || receiptSent}/> </div>
                    <div className="grid grid-cols-2 gap-3">
                        <Button variant="outline" className="h-12" onClick={() => handleSimulatedSendReceipt('SMS')} disabled={isProcessingOrSending || receiptSent || !isReceiptRecipientValidPhone || !receiptRecipient}> {isProcessingOrSending && !receiptSent ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <MessageSquare className="mr-2"/>} Send SMS </Button>
                        <Button variant="outline" className="h-12" onClick={() => handleSimulatedSendReceipt('Email')} disabled={isProcessingOrSending || receiptSent || !isReceiptRecipientValidEmail || !receiptRecipient}> {isProcessingOrSending && !receiptSent ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <Mail className="mr-2"/>} Send Email </Button>
                    </div>
                </div>
                <Button variant="secondary" onClick={handleNoReceipt} className="w-full h-12 text-base mb-3" disabled={isProcessingOrSending || receiptSent}> No Receipt </Button>
                <Button onClick={handleFinalizeSale} className="w-full h-16 text-xl mt-auto" disabled={isProcessingOrSending || !receiptSent}>
                    {isProcessingOrSending && receiptSent ? <Loader2 className="mr-2 h-6 w-6 animate-spin" /> : "Finalize Sale"} </Button>
            </>
        )}
      </div>
    </div>
